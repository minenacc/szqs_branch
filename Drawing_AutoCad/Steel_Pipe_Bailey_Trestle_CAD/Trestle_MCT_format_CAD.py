...
from Drawing_AutoCad.Steel_Pipe_Bailey_Trestle_CAD.Trestle_MCT_pre_CAD import (
    ...
    build_bailey_diaver_nodes, collect_all_bailey_nodes, build_distribute_nodes, build_pile_nodes, build_connect_x_nodes, build_virtual_beam_nodes,
    ...
    build_bailey_diagonal_elements, build_distribute_elements, build_pile_elements, build_connect_x_elements, build_virtual_beam_elements, build_move_load_and_stldcase,
    build_structure_groups, build_elastic_links, build_FRAME_lst, build_constraint_lines, build_line_lanes, build_vehicle_lines, build_wind_load, build_water_force_load, 
    build_mvldcase_lines, build_loadcomb_lines
)
...
def build_mct_from_ui(app):
    ...
    def convert_section_data_for_cad(names, cad_params, weights):
        ...
        zd_f1_section_cad      = safe_get(cad_params, 5, {})
        zd_f1_section_name     = safe_get(names, 5, None)
        zd_f2_section_cad      = safe_get(cad_params, 6, {})
        zd_f2_section_name     = safe_get(names, 6, None)
        zd_section_cad         = safe_get(cad_params, 4, {})
        zd_section_name        = safe_get(names, 4, None)
        ...
        return (
            ...
            zd_f1_section_cad, zd_f1_section_name,
            zd_f2_section_cad, zd_f2_section_name,
            zd_section_cad, zd_section_name,
    ...
    print(zd_f1_section_name)
    print(zd_f1_section_cad)
    print(zd_f2_section_name)
    print(zd_f2_section_cad)
    print(zd_section_name)
    print(zd_section_cad)  
    ...
    (       
            ...
            bailey_bottom_abutment_nodes,    # 桥台固结点
    ...
    bailey_diaver_nodes, bailey_diaver_single_nodes, node_id = build_bailey_diaver_nodes(
            bailey_top_vertical_nodes, z_top, node_id
        )
    (
            all_nodes_bailey,
            single_nodes,
    ...
    print(bailey_bottom_brake_elink_nodes)
    # 钢管桩
    (
            pile_top_nodes,
            pile_connect_nodes,           
            pile_bottom_nodes,
            pile_group_nodes,
            brake_pile_top_nodes,
            brake_pile_connect_nodes,      
            brake_pile_bottom_nodes,
            brake_pile_group_nodes,
            water_force_points_by_y,         
            water_force_points_brake_by_y,    
            node_id
        ) = build_pile_nodes(
            components_tab,
            distribute_pile_elink_nodes=distribute_pile_elink_nodes,  
            brake_f2_pile_elink_nodes=brake_f2_pile_elink_nodes,      
            start_id=node_id,
            section_data=section_data,
            load_tab=app.tab_loads
        )
    # 联结系
    connect_x_nodes, connect_groups, brake_connect_nodes, brake_connect_groups, node_id = build_connect_x_nodes(
            components_tab,
            pile_group_nodes,
            pile_connect_nodes,        
            brake_pile_group_nodes,
    ...
    # 虚拟梁
    virtual_nodes, virtual_single_nodes, node_id = build_virtual_beam_nodes(
        xl_virtual_elink_nodes, section_data, node_id)
    # 节点数据汇总
    def _flatten_group_nodes(group_dict: dict) -> dict:
            ...
            for _, lst in group_dict.items():
                for nid, coord in lst:
                    flat[nid] = coord
            return flat
    brake_f1_nodes_flat    = _flatten_group_nodes(brake_f1_nodes)
    brake_f2_nodes_flat    = _flatten_group_nodes(brake_f2_nodes)
    ...
    | brake_f2_nodes_flat
    | pile_nodes_flat
    | brake_pile_nodes_flat
    | connect_nodes_flat
    | brake_connect_nodes_flat
    | virtual_nodes
    )
    ...
    distribute_elements, elem_id = build_distribute_elements(
        ...
        distribute_single_nodes, brake_f1_nodes, brake_f2_nodes, elem_id
    )
    # 钢管桩
    pile_elements, elem_id = build_pile_elements(
        section_data, name_to_id,
        pile_group_nodes, brake_pile_group_nodes, elem_id
    )

    # 联结系
    connect_elements, elem_id = build_connect_x_elements(
        section_data, name_to_id, connect_groups, brake_connect_groups, elem_id
    )
    # 虚拟梁
    virtual_elements, elem_id = build_virtual_beam_elements(
        virtual_single_nodes, section_data, name_to_id, elem_id
    )
    ...
    elink_lines = build_elastic_links( 
        ...
        # 3) 制动墩相关（可选）
        bailey_bottom_brake_elink_nodes, # {nid: (x,y,z)}
        brake_f1_bottom_elink_nodes,     # {nid: (x,y,z)}
        brake_f1_f2_elink_nodes,        # {nid: (x,y,z)}
        brake_f2_f1_elink_nodes,        # {nid: (x,y,z)}
        brake_f2_pile_elink_nodes,      # {nid: (x,y,z)}
        # 4) 分配梁-普通桩
        distribute_pile_elink_nodes,    # {nid: (x,y,z)}
        pile_top_nodes,                 # {nid: (x,y,z)}
        # 5) 制动墩 F1-制动墩桩
        brake_pile_top_nodes,           # {nid: (x,y,z)}
        # 6. 虚拟梁与小肋
        xl_virtual_elink_nodes,
        virtual_nodes,
        start_link_id = 1
    ...
    for line in ELEMENT_lst:
        ...
        for rn in release_nodes:
            ...
            if rn == node2:
                found_in_J += 1
    print(f"[调试] release_nodes 出现在 I端 {found_in_I} 次，J端 {found_in_J} 次")

    FRAME_lst = build_FRAME_lst(ELEMENT_lst, release_nodes)
    print("释放梁端约束生成完成")
    
    # 桩底固结
    CONSTRAINT_lst = build_constraint_lines(pile_bottom_nodes, brake_pile_bottom_nodes,bailey_bottom_abutment_nodes)
    print("桩底固结生成完成")
    print("约束生成完成")
    ...
    # 荷载组及荷载说明
    load_tab = app.tab_loads
    move_load_lst, move_STLDCASE_lst = build_move_load_and_stldcase(load_tab)
    print("荷载组生成完成")

    # 风荷载
    print("[调试] 桩顶节点号:", list(pile_top_nodes.keys()))
    WIND_GROUP_LINES,Feng_values,Feng_result_lst = build_wind_load(load_tab, components_tab,pile_top_nodes=pile_top_nodes, brake_pile_top_nodes=brake_pile_top_nodes)
    print("风荷载生成完成")

    # 水流力
    WATER_FORCE_LINE, Shui_values, Shui_lst= build_water_force_load(load_tab, section_data, components_tab, water_force_points_by_y,water_force_points_brake_by_y)
    print("水流力生成完成")
    ...
    code_lines = [
            "*MVLDCODE",
            "   CODE=CHINA"]
    lane_lines =build_line_lanes(load_tab, virtual_elements)
    vehicle_lines = build_vehicle_lines(load_tab)
    mvldcase_lines = build_mvldcase_lines(load_tab)
    MOVE_LOAD = code_lines + lane_lines + vehicle_lines + mvldcase_lines
    print("移动荷载生成完成")

    # 荷载组合
    LOADCOMB_LINES = build_loadcomb_lines(load_tab)
    print("荷载组合生成完成")
    ...
    mct_lst = [
                ...
                # 荷载说明
                ["*STLDCASE", "自重, USER,", "风荷载, USER,", "水流力, USER,"] ,
                # 自重

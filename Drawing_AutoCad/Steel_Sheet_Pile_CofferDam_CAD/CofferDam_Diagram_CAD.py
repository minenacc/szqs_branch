...
class RealTimeDiagram:
    ...
    def __init__(self, parent, var_dict, soil_load_dict, waler_rows,
        ...
        """var_dict 包含所有 tkinter Variable 的引用"""
        self.parent = parent
    ...
    def register_group(self, group_name, widgets):
        """注册一组控件到高亮组, widgets 为单个 widget 或 widget 列表"""
    ...
    def _get_strut_elev_width(self, name):
        """读取内支撑截面宽度 (mm→m) for elevation view"""
        if not name or name == '/':
            return 0.3
    ...
    def _draw_plan(self, highlight):
        ...
        def _strut_w(sec):
            """从 Strut_section_dict 读取截面宽度 (mm→m)"""
...
class StageDiagram:
    """施工阶段示意图 — 读取前3个Tab缓存绘制立面图底图 + 工况叠加内容"""
    ...
    def _update_state(self, stage_dict, target_idx):
        """从第1个工况开始逐个应用到 target_idx，累积 current_state"""
        state = {
            "soil_top": None,
            "water": None,
            "supports": {},
    ...
    def _get_support_section(self, layer):
        """获取指定支撑层的截面信息（从缓存的 waler_rows 中查找）"""
        if not self.sheet_refs:
            return [], 0.0

...
from General.AutoCAD     import find_installed_autocad_versions, AutoCAD_VersionR_tans_to_COM_ProgID, Import_Blocks, get_acad_application, ensure_acad_document, prepare_acad, register_acad_message_filter

sys.path.append(str(Path(__file__).parent))
from Drawing_AutoCad.Steel_Sheet_Pile_CofferDam_CAD.CofferDam_Diagram_CAD   import RealTimeDiagram
from Drawing_AutoCad.Steel_Sheet_Pile_CofferDam_CAD.CofferDam_Calculate_CAD import calc_skggz_center_spacing
from Drawing_AutoCad.Steel_Sheet_Pile_CofferDam_CAD.CofferDam_Generate_CAD  import Steel_Sheet_Pile_CofferDam_on_submit_DWG
from Drawing_AutoCad.Steel_Sheet_Pile_CofferDam_CAD.CofferDam_Excel_io_CAD  import load_all_params, save_to_excel, get_excel_path, PROJECT_TEMPLATE, global_connections, load_waler_strut_sections, load_material_library
...
def _centered_messagebox(msg_method, message, title, parent, **kwargs):
    """在屏幕居中、置顶显示 ttkbootstrap Messagebox 弹窗"""
    try:
        sw = parent.winfo_screenwidth()
        sh = parent.winfo_screenheight()
        lines = message.count('\n') + 1
        est_w = min(max(len(message) * 8, 320), 600)
        est_h = 100 + lines * 20
        kwargs['position'] = ((sw - est_w) // 2, (sh - est_h) // 2)
    except Exception:
        pass
    # 临时将父窗口设为置顶，确保弹窗显示在最前面
    try:
        parent.attributes('-topmost', True)
    except Exception:
        pass
    msg_method(message, title=title, parent=parent, **kwargs)
    try:
...
def Steel_Pile_CofferDam_Dialog_CAD(root, acadapp, Applocation):
    ...
    # 局部导入 Tab 函数（避免循环依赖）
    from CofferDam_Tab_CAD import (
        Basic_Setting_GUI, Waler_Strut_Concrete_Setting_GUI,
        Soil_Info_Setting_GUI, _load_soil_into_frame,
    )
    ...
    print(f"[Debug] 围檩截面列表({_excel_waler_secs[:5]}...共{len(_excel_waler_secs)}项)")
    print(f"[Debug] 内支撑截面列表({_excel_strut_secs[:5]}...共{len(_excel_strut_secs)}项)")
    Steel_dict, Concrete_dict, Rebar_dict = load_material_library(_excel_path)

    # ---- 全局变量初始化（默认值，项目切换会覆盖） ----
    Solid_Level = tb.DoubleVar(value=0.0)
    Water_Level = tb.DoubleVar(value=0.0)
    ...
    def validate_all_inputs():
        """检查所有标签页输入完整性, 全面拦截 TclError/ValueError, 收集所有错误一次性全报"""
        errors = []
        ...
        # # 3.3 牛腿数量检查：有围檩时不允许为"/"
        # has_waler = any(r.get('combo2', tb.StringVar()).get() not in ('', '/')
        #                 or r.get('combo3', tb.StringVar()).get() not in ('', '/')
        #                 for r in Waler_rows)
        # if has_waler:
    ...
    def _validate_all_projects():
        """全局项目数据校验 [项目名+编号]"""
        # from CofferDam_Excel_io_CAD import parse_stage_data
        all_errors = []
        for pk, pv in projects_dict.items():
    ...
    #     try: ctx['water_level'] = float(Water_Level.get())
    #     except: ctx['water_level'] = 0.0
    #     ctx['water_above_ground'] = ctx['water_level'] > ctx['ground_level']
    ...
    #     """将 sk_dict 序列化回 proj 的 Excel 字段"""
    ...
    def combine_cofferdam_dict():
        """纯读取函数：读取当前 UI 全部数据，写入 projects_dict[current_key]

        读取流程：
        1. 调用各 Tab 的 refresh_cache → 更新各 Tab 的 cache dict
        2. 从各 cache 提取数据 → 写入 projects_dict[current_key]（固定格式）
        """
        ...
        for refresh_key in ["refresh_basic", "refresh_substructure"]:
    ...
    def param_save():
        ...
        if cross_errors:
            ...
            return
        #（调试信息统一在 Steel_Sheet_Pile_CofferDam_on_submit_DWG 中输出）
    ...
    def _dwg_generate():
        ...
        if not ok:
            ...
            return
        # 土层保存验证（采用加权平均土层时，常规土层可为空，跳过校验）
        _wadopt_dwg, *_ = _weighted_ui_values()
        if not _wadopt_dwg:
            combine_cofferdam_dict()
            _proj_dwg = projects_dict.get(_current_proj_key[0], {})
            _soil_ok, _soil_msg = _validate_soil_for_save(_proj_dwg)
            if not _soil_ok:
        ...
        #         '承台短边高差(m)': item.get("diff_short", "/"),
        #     }
        # 激活 CAD 窗口置顶，提示用户准备在 CAD 中操作
        try:
            import win32gui
            import win32con
            hwnd = win32gui.FindWindow(None, acadapp.Caption)
            if hwnd:
            ...
            pass
        # 支护桩材质：优先取详细设置中保存的材质，兜底取项目参数
        _pile_type = Sheet_Pile_typevar.get()
        if _pile_type == '钢板桩':
            _sheet_pile_material = (sheet_refs.get("_last_gbz_material") if sheet_refs else "") or \
                (projects_dict.get(_current_proj_key[0], {}).get("支护桩材质", "") or "Q295")
        elif _pile_type == '锁扣钢管桩':
            _sheet_pile_material = (sheet_refs.get("_last_skggz_material") if sheet_refs else "") or \
                (projects_dict.get(_current_proj_key[0], {}).get("支护桩材质", "") or "Q235")
        else:
            _sheet_pile_material = "Q235"

        # ---- 生成DWG：自动重试/自动重连，CAD繁忙/关闭切换类错误不再抛给用户 ----
        import time
        import pywintypes
        import win32com.client

        def _activate_cad(app):
            try:
                import win32gui
                import win32con
                hwnd = win32gui.FindWindow(None, app.Caption)
                if hwnd:
                    win32gui.SetWindowPos(hwnd, win32con.HWND_TOP, 0, 0, 0, 0,
                                          win32con.SWP_NOMOVE | win32con.SWP_NOSIZE)
                    win32gui.ShowWindow(hwnd, win32con.SW_RESTORE)
        ...
        def _run_dwg(app):
            Steel_Sheet_Pile_CofferDam_on_submit_DWG(
                app,
                Cap_X.get(), Cap_Y.get(), X_offset.get(), Y_offset.get(), Cap_H.get(),
                Sheet_Pile_typevar.get(), CofferDam_L.get(), Sheet_Pile_namevar.get(),
                Pile_SEC_info_dict, SKGGZ_D.get(), SKGGZ_t.get(),
                SKGGZ_LockWidth.get(), SKGGZ_SheetCount.get(), SKGGZ_SheetWidth.get(),
                Waler_rows, Strut_blocks,
                Solid_Level.get(), Water_Level.get(),
                Cap_Bottom_Level.get(), CofferDam_Top_Level.get(),
                Concrete_Blinding_check_var.get(), Concrete_Blinding_grade_var.get(), Concrete_Blinding_thickness_var.get(),
                Concrete_Plug_check_var.get(), Concrete_Plug_grade_var.get(), Concrete_Plug_thickness_var.get(),
                Waler_section_dict, Strut_section_dict,
                recognize_Cap_result_dict, recognize_Waler_result_dict,
                recognize_Strut_result_dict, load_dict_for_gen,
                Sheet_Pile_materialvar=_sheet_pile_material,
                CofferDam_No=project_no_combo.get(),
            )

        # ---- 连接/图纸/预定义块准备 + 绘图：各只执行一次，不循环、不重复弹交互 ----
        # 注册 COM 消息过滤器：CAD 忙时自动重试"被呼叫方拒绝"(RPC_E_CALL_REJECTED)，不抛错
        register_acad_message_filter()
        _ok = False
        _last_err = None
        try:
            _app, _doc, _msp = prepare_acad(create_if_none=True)
            # 关键：新建图纸里没有预定义块，需补导（已存在则跳过）
            Import_Blocks(Applocation, _app)
            _activate_cad(_app)
            _run_dwg(_app)
            _ok = True
        except pywintypes.com_error as _e:
            _last_err = _e
            print("CAD 绘图调用失败:", _e)
            ...
            print("CAD 绘图异常:", _e)
        if _ok:
            _centered_messagebox(Messagebox.show_info,
                                "CAD 绘图已完成，请切换至 AutoCAD 查看。",
                                title="绘图完成", parent=root)
        else:
            if _last_err is not None:
                print("CAD 绘图最终失败:", _last_err)
            _centered_messagebox(Messagebox.show_warning,
                                "AutoCAD 连接/图纸准备失败或绘图未完成，请确认已打开/新建图纸后重试。",
                                title="绘图未完成", parent=root)
    ...
    def _switch_project(proj_key):
        ...
        Water_Level.set(_as_float(proj.get("初始水位标高(m)", 0)))
    ...
    def _init_project_selection():
        """初始化项目下拉：选中 last_proj_key 对应的项目名和编号，并加载数据"""
    ...
    mct_btn = tb.Button(btn_frame, text='生成DWG', command=_dwg_generate, bootstyle=SUCCESS, width=10)
    mct_btn.pack(side=LEFT, padx=4)
...
def auto_calculate_pile_offsets(Cap_X, Cap_Y, X_offset, Y_offset,
                                 Sheet_Pile_typevar, Pile_SEC_info_dict,
                                 SKGGZ_D, SKGGZ_Gap):
    """根据承台尺寸和支护桩截面参数自动计算承台边距

    承台边距 = (围堰轮廓尺寸 - 承台尺寸) / 2
    围堰轮廓尺寸根据支护桩类型和截面参数调整为桩数量取整后的结果。
    长边（围堰长边方向）奇数桩数，短边（围堰宽边方向）偶数桩数。
    """
    try:
        cap_x_m = Cap_X.get()  # m
        cap_y_m = Cap_Y.get()  # m
...
def CofferDam_Drawing_AutoCad_Main(parent=None, on_close=None):
    ...
    if not if_Reg(parent):
        if on_close:
            on_close()
        return
    # 注册 COM 消息过滤器：CAD 忙时自动重试"被呼叫方拒绝"，避免 COM 报错
    register_acad_message_filter()
    ...
    try:
        for Rx in versionR_lst:
            try:
                ...
                pass
        Applocation = read_Register(f'Software\\{FileName}', 'Applocation') # 软件安装路径
        Import_Blocks(Applocation, acadapp)
        if parent == None:
            root = tb.Window("钢板桩围堰自动化绘图助手")
        else:
            root = tb.Toplevel(parent)
            root.title("钢板桩围堰自动化绘图助手")
        Steel_Pile_CofferDam_Dialog_CAD(root, acadapp, Applocation)
        if parent == None:
            root.mainloop()
        elif on_close:
...
#     CofferDam_Drawing_AutoCad_Main()
